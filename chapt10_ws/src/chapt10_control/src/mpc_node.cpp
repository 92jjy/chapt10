 ...
 * MIT License
 *
 *   https://github.com/kuralme/mpc_ros2
 *   Copyright (c) 2024 Mustafa Ege Kural, MIT License
 *
 */
#include <chrono>
#include <cmath>
...
#include "chapt10_control/mpc.hpp"

static const rclcpp::Logger LOGGER = rclcpp::get_logger("mpc_node");
...
class MPCRosNode : public rclcpp::Node
{
  public:
    ...
    void globalPathCallback(const std::shared_ptr<const nav_msgs::msg::Path>& globPathMsg);
  ...
  private:
    ...
    std::string mpcPath_Otn_, mpcCmd_Otn_, path_Itn_;
    std::string mapFrame_, odomFrame_, robotFrame_;

    geometry_msgs::msg::Pose odom_;
    double linVel_{0.0};
    geometry_msgs::msg::Point globalPathEnd_;
    ...
    rclcpp::Subscription<nav_msgs::msg::Path>::SharedPtr globalPathSub_{nullptr};
  ...
  : Node(nodeName, options), isGoalReached_(false)
{
  ...
  this->get_parameter_or<std::string>("path_topic", path_Itn_, "/plan");
  ...
  this->get_parameter_or<std::string>("robot_frame", robotFrame_, "base_footprint");
  ...
  std::cout << "- Path topic      : " << path_Itn_ << std::endl;
  ...
  // Create ROS2 subscriptions and publishers
  ...
  globalPathSub_ = this->create_subscription<nav_msgs::msg::Path>(
    path_Itn_, 10, std::bind(&MPCRosNode::globalPathCallback, this, std::placeholders::_1));
  ...
  std::cout << "mpc_ref_vel    : " << refVel     << std::endl;
  std::cout << "mpc_ref_cte    : " << refCte     << std::endl;
  ...
  _mpc = std::make_shared<MPC>(mpc_params);
}
...
void MPCRosNode::globalPathCallback(const std::shared_ptr<const nav_msgs::msg::Path>& globPathMsg)
{
  odomGlobalPath_ = downsamplePath(*globPathMsg);
  isGoalReached_  = false;
  linVel_         = 0.0;
  if (!globPathMsg->poses.empty())
  {
    globalPathEnd_ = globPathMsg->poses.back().pose.position;
  }
}

/**
* @brief MPC global path downsampling
...
nav_msgs::msg::Path MPCRosNode::downsamplePath(const nav_msgs::msg::Path& globPathResponse)
{
  ...
  try {
    ...
    for (size_t i = 0; i < pathSize; i++)
    {
      ...
      if (downSampleSize && sample % downSampleSize == 0)
      {
    ...
    RCLCPP_WARN(
    this->get_logger(), "Could not transform %s to %s: %s (use the path as is)",
      odomFrame_.c_str(), pathFrame.c_str(), ex.what());
    return globPathResponse;
  }
...
void MPCRosNode::calculateControl(void)
{
  if (!isGoalReached_)
  {
    ...
    try
    {
      tf_odom2base = tfBuffer_->lookupTransform(odomFrame_, robotFrame_, tf2::TimePointZero);
    }
    catch (const tf2::TransformException &ex)
    {
    ...
    odom_.position.x = tf_odom2base.transform.translation.x;
    odom_.position.y = tf_odom2base.transform.translation.y;
    odom_.orientation = tf_odom2base.transform.rotation;

    const double goalDist = std::hypot(globalPathEnd_.x - odom_.position.x,
                                       globalPathEnd_.y - odom_.position.y);
    if (goalDist < goalRadius_)
    {
      isGoalReached_ = true;
      std::cout << "Goal Reached! (dist = " << goalDist << " m)" << std::endl;
    ...
    double throttle = 0.0;
    double angVel   = 0.0;
    const double linVel = linVel_;
    Eigen::VectorXd state(6);
    if(delayMode_)
    {
      // Kinematic model is used to predict vehicle state at the actual moment of control
      const double px_act = linVel * dt_;
      const double py_act = 0;
      const double theta_act = angVel * dt_;
      const double v_act = linVel + throttle * dt_;
    ...
    */
    ...
    if (mpcTraj.empty() || mpcCommands.empty()) {
      ...
      linVel_ += throttle * dt_; // Linear velocity
      if (linVel_ >= maxSpeed_)
        linVel_ = maxSpeed_;
      if(linVel_ <= 0.0)
        linVel_ = 0.0;

      mpcCommands_.linear.x  = linVel_;
      mpcCommands_.angular.z = angVel;

      mpcTraj_.poses.clear();
      mpcTraj_.header.frame_id = odomFrame_;
...
int main(int argc, char** argv)
{
  ...
  auto mpc_node = std::make_shared<MpcRos::MPCRosNode>("mpc_node", options);

  // Extra thread to handle ROS2 events
  auto spin_thread = std::make_unique<std::thread>([&executor, &mpc_node]() {
    executor.add_node(mpc_node->getNodeBaseInterface());
    executor.spin();
    executor.remove_node(mpc_node->getNodeBaseInterface());
  });
